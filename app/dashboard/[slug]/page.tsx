...
function HelpTooltip({ text }: { text: string }) {
  return (
    <div className="relative group shrink-0">
      ...
      <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:right-0 top-10 z-50 hidden w-72 rounded-lg border border-white/10 bg-[#000000] p-5 text-sm font-light leading-relaxed text-white/90 shadow-2xl group-hover:block backdrop-blur-3xl">
        {text}
      </div>
    </div>
  );
...
export default async function DashboardPage({ params, searchParams }: PageProps) {
  ...
  // AQUÍ ESTÁ A CORREÇÃO: Sintaxe limpa e perfeita.
  ...
  return (
    <main className="min-h-screen bg-black text-white p-8 md:p-16 font-sans selection:bg-white selection:text-black relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto space-y-24 relative z-10">
        ...
        {/* HERO SECTION: BRUTALISTA, SEM CAIXA */}
              ...
              <div className="space-y-8">
                {rank.data.slice(0, 5).map((s, idx) => (
                  <div key={idx} className="flex justify-between items-end mb-4 group border-b border-white/5 pb-4">
                    ...
                    <span className={`text-3xl font-bold tracking-tighter ${s.count > 0 ? "text-[#E5D3B3]" : "text-white/10"}`}>{s.count}</span>
                  </div>
                ))}
              </div>
            </div>
